...
- **Auto-connect on boot**: `VINYL_AUTO_CAST` (priority name list, default `Living Room pair`). Hooked into the auto-refresh scan loop in `fullscreen_ui.py` (`_maybe_auto_cast`) via the pure matcher `select_preferred_target` in `group_discovery.py`; reuses the normal cast path, fires once per boot, never overrides a manual choice.
